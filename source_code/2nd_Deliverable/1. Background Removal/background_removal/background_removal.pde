...
void setup() {
  ...
  backgroundReplacement = loadImage("backgroundReplacement.jpg");
}
...
void draw() {
  ...
  for (int x = 0; x < video.width; x ++ ) {
    for (int y = 0; y < video.height; y ++ ) {
      ...
      float r2 = blue(bgColor);
      float r3 = green(bgColor);
      float g1 = green(bgColor);
      float diff = dist(r1, g1, r2, r3 );

      // Step 5, Is the foreground color different from the background color
      if (diff > threshold) {
        // If so, display the foreground color
        pixels[loc] = backgroundReplacement.pixels[loc];
      } else {
